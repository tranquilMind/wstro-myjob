...
<head>
    ...
    <link rel="stylesheet" href="${rc.contextPath}/statics/common/layui/css/layui.css">
    <style>
...
<script src="${rc.contextPath}/statics/common/layui/layui.js"></script>
<script>
    var id = "";
    jQuery.post("${rc.contextPath}/chat/imController.do?getUserid", {
    ...
    layui.use('layim', function(layim){
       //建立WebSocket通讯
        var socket = new WebSocket("ws://localhost:8080/wstro/WebSocket/"+id);

        //基础配置
        layim.config({
            //初始化接口
            init: {
                url: '${rc.contextPath}/chat/imController.do?getUsers'
                ,data: {}
            }
            ...
            //查看群员接口
            ,members: {
                url: '${rc.contextPath}/demo/json/getMembers.json'
                ,data: {}
            }
			//update-begin--author:zhangjiaqiang Date:20161031 for:在线聊天（图片、文件、聊天记录）功能的实现
            ,uploadImage: {
                url: '${rc.contextPath}/chat/imController.do?uploadImage' //（返回的数据格式见下文）
                ,type: 'post' //默认post
            }

            ,uploadFile: {
                url: '${rc.contextPath}/chat/imController.do?uploadFile' //（返回的数据格式见下文）
                ,type: '' //默认post
            }
            ,title:"在线聊天"
            ,maxLength:3000
            ...
            ,chatLog: '${rc.contextPath}/chat/chatMessageHistory.do?from='+id //聊天记录地址
